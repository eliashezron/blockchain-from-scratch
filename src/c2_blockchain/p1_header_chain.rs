...
use crate::{c3_consensus::Consensus, hash};
...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        // iterate through the blocks verifying if the children block heights are the same as the parent block height + 1 and the parent block hash is the same as the block has of the child block
        let mut current = self.clone();
        for block in chain {
            if block.parent != hash(&current) || block.height != current.height + 1 {
                return false;
            }
            current = block.clone();
        }
